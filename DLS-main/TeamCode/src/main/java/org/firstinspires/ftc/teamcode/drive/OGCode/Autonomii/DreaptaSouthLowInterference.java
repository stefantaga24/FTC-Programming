...
public class DreaptaSouthLowInterference extends LinearOpMode {
    enum STROBOT
    {
        START,
        GET_LIFT_DOWN,
    ...
    public static double x_PLACE_SOUTH_HIGH = 13.5, y_PLACE_SOUTH_HIGH = -16, Angle_PLACE_SOUTH_HIGH = 20;
    public static double x_PLACE_SOUTH_HIGH_PRELOAD = 12.5, y_PLACE_SOUTH_HIGH_PRELAOD = -18, Angle_PLACE_SOUTH_HIGH_PRELOAD = 20;
                    ...
                    {OKEIUT=0;
                        if (nr==0)
                        {
                            if (!drive.isBusy()) {
                                ...
                                timerLift.reset();
                                status = DreaptaSouthLowInterference.STROBOT.GET_LIFT_DOWN;
                            }
                        }
                        else {
                            ...
                            else if(autoControllerTurn51.CurrentStatus == FARA_CON)
                            {
                               ...
                               // else { nr =1; autoControllerTurn51.Cone_Stack_Level =  autoControllerTurn51.Cone_Stack_Level +1;}
                                status = DreaptaSouthLowInterference.STROBOT.GO_TO_COLLECTING_POSITION;

                            }
                        }
                    }
                    else {
                        status = DreaptaSouthLowInterference.STROBOT.PARK;
